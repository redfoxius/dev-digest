...
Plan, `docs/<slug>-plan.md`); кожен його Work Item посилається на `AC-N`
зі специфікації. `implementation-planner` більше не уточнює вимоги сам —
якщо специфікація відсутня або містить незакритий
`[NEEDS CLARIFICATION]`, він зупиняється й звітує про це, а не питає
користувача. `researcher`, `implementation-planner`, `plan-verifier` і
`architecture-reviewer` не пишуть код — вони віддають артефакт (звіт,
план або знахідки) назад у сесію, яка їх викликала.
`implementation-planner` не зберігає план сам — за конвенцією
