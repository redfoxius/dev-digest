...
### Post-review fixes, round 2: newly-surfaced items (missing sibling indexes, then all remaining SUGGESTIONs)

Re-checking the fixed files surfaced 3 more instances of the same missing-FK-index pattern already fixed for `pr_commits.pr_id`: `reviews.pr_id` (migration `0020`), `findings.review_id` (migration `0020`), and `pr_files.pr_id` (migration `0021`) — all pre-existing gaps this PR didn't introduce, fixed on explicit request. Each was independently re-verified.

The remaining SUGGESTION-tier findings were then fixed on request too:

1. **`renderIntentText` placement** (`onion-architecture`) — moved from `server/src/modules/reviews/helpers.ts` to `reviewer-core/src/prompt.ts` (exported via `reviewer-core/src/index.ts`); pure `Intent → string` rendering belongs with the `## Derived intent` prompt section it feeds, and `reviewer-core` already imports plenty of `@devdigest/shared` types (`Finding`, `Review`, etc.), so this isn't a new dependency precedent. `helpers.ts` keeps a one-line re-export (matching its existing `reduceReviews`/`sliceDiff` re-export convention) so `run-executor.ts`'s import didn't need to change.
2. **Missing `.min(1)` on `in_scope`/`out_of_scope` array items** (`zod`) — added, matching `intent`'s existing `.min(1)`.
3. **Missing response schemas on the two intent routes** (`fastify-best-practices`) — added `response: { 200: PrIntentRecord.nullable() }` (GET) / `response: { 200: PrIntentRecord }` (POST), reusing the existing `PrIntentRecord` zod contract. No test previously exercised these two routes at all, so a new integration test (`reviews.it.test.ts`, "Intent Layer routes satisfy their new response schema") was added specifically to prove the schema doesn't reject the service's actual return shape — response-schema additions are exactly the kind of change that can 500 at runtime despite a clean typecheck if the shapes drift, so this wasn't optional diligence. That test surfaced a real, separate gap: `MockIntentDeriver` (`server/src/adapters/mocks.ts`) doesn't replicate the real `IntentDeriverService`'s internal `upsertIntent` call, so a GET-after-derive round-trip isn't testable through the mock — noted in the test as a mock-fidelity gap, not fixed (out of scope for this round, not requested).
4. **`pr_intent_confidence_range` CHECK added without `NOT VALID`** (`postgresql-table-design`) — split into `... CHECK (...) NOT VALID` + a separate `VALIDATE CONSTRAINT` in migration `0017` (safe to hand-edit for the same reason established in round 1: timestamp-based migration tracking, not content-hash).
5. **`CREATE INDEX` not built `CONCURRENTLY`** (`postgresql-table-design`) — deliberately NOT fixed. Checked: Postgres forbids `CREATE INDEX CONCURRENTLY` inside a transaction block, and drizzle-orm's migrator (`pg-core/dialect.cjs`) wraps every migration in one shared transaction with no per-statement opt-out — using `CONCURRENTLY` here would make every future `db:migrate` run fail outright. Documented with a comment in migration `0019` instead of forced; this is a genuine tooling constraint, not a style call.

Verification re-run again: `pnpm typecheck` clean in `server`/`reviewer-core`; `server` unit (288 tests) + full integration suite (12 files, 66 tests including the new one, fresh testcontainers Postgres) all green; `reviewer-core` (30 tests) green.
