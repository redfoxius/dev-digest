...
- Status: draft — all `[NEEDS CLARIFICATION]` items resolved (chunking strategy in §9, indexing/injection size caps in §7).
- Version: 0.1
...
- Related: 4 mockup screenshots (Project Context page, Agent Editor Context tab,
  ...
  (the UI precedent the new Project Context page mirrors), no implementation
...
documents (discovered under a repo's `specs/`, `docs/`, and `insights/`
folders, or any other configured search root) to specific agents and skills.
...
| Term | Definition |
|---|---|
| Search root | A glob pattern (default `**/{specs,docs,insights}/**/*.md`) a repo is scanned against to discover attachable documents. Configurable per repo. |
| Discovered document | One markdown file found by the most recent scan of a repo's search roots — tracked in `context_documents`, identified by its repo-relative path. |
...
file found under the repo's configured search roots (default
`specs/`, `docs/`, `insights/`), grouped by root, with an "Indexed: N files ·
M chunks · last refreshed …" status line and a manual "Reindex" action.
Selecting a document shows its rendered content in a read-only Preview pane.
...
flowchart TD
    subgraph Indexing["Indexing & browsing (Project Context page)"]
        direction TD
        FS[("Repo clone\nrepos.clonePath")] -->|glob scan| Reader[Reader: recursive .md discovery]
...
- AC-1 (Event-driven): WHEN a reindex is triggered for a repo, the system shall recursively scan the repo's cloned working tree against its configured search-root globs and upsert one `context_documents` row per matched markdown file (path, root, size, content hash). Verify: `POST /repos/:repoId/context-docs/reindex` against a fixture clone with 6 markdown files across `specs/`, `docs/`, `insights/` returns a document list of 6 with correct `root` values.
...
- AC-3 (Ubiquitous): The system shall derive a document's `root` classification (`specs`/`docs`/`insights`) solely from which configured search-root glob matched its path — never a separate content-based classification step. Verify: a file at `docs/architecture.md` is tagged `root: "docs"` regardless of its content.
- AC-4 (Ubiquitous): The system shall exclude paths ignored by the repo's own `.gitignore` (e.g. `node_modules/**`) from discovery, mirroring repo-intel's existing scan exclusions. Verify: reindex against a fixture repo with a `node_modules/pkg/README.md` file confirms it is absent from `context_documents`.

### 6.2 Search-root configuration (per repo, user-configurable)
- AC-5 (Where-optional): WHERE a repo has no explicit `context_search_globs` configured, the system shall use the default glob `**/{specs,docs,insights}/**/*.md`. Verify: `GET /repos/:repoId/context-config` for a freshly onboarded repo returns `{globs: ["**/{specs,docs,insights}/**/*.md"]}`.
- AC-6 (Event-driven): WHEN a user updates a repo's search-root config via `PUT /repos/:repoId/context-config`, the system shall persist the new glob list scoped to that repo only and apply it on the next reindex. Verify: PUT a custom glob, trigger reindex, confirm discovered documents reflect only the new glob's matches; a second repo's config and documents are unaffected.
- AC-7 (Unwanted behavior): IF a configured glob would resolve outside the repo's own `clonePath` (e.g. contains `../` or an absolute path), THEN the system shall reject it with a `422` on `PUT /repos/:repoId/context-config` rather than persisting it. Verify: `PUT` with `["../../etc/**/*.md"]` returns 422; no file outside `clonePath` ever appears in `context_documents`.
...
- AC-41 (Ubiquitous): The system shall never read or index a file outside the repo's own cloned working tree regardless of the configured glob (covered structurally by AC-7's `422` rejection of any glob resolving outside `clonePath`). Verify: same test as AC-7.
...
| AC-7 | Configured glob attempts to escape `clonePath` | 6 (Edge Cases — malformed input) / 5 (Security) |
...
| `root` | enum `specs` \| `docs` \| `insights` | Derived from the matched search-root glob (AC-3). |
...
`'code' | 'docs' | 'spec'`) so all three search roots have a matching value;
no other schema change. **Chunking strategy: split on markdown headings**
...
**New column: `repos.context_search_globs`** — `text[]`, nullable.
`null` → default glob `**/{specs,docs,insights}/**/*.md` applies (AC-5); a
non-null array is the repo's own configured roots (AC-6).
...
**`GET /repos/:repoId/context-config`** / **`PUT /repos/:repoId/context-config`** → request/response `{ globs: string[] }`; `PUT` responds `422` on an escaping glob (AC-7).
...
reindex or check search-root config), error (reindex/API failure —
...
or plant a document under a search root before it's attached). It is treated
...
- [ ] AC-1 — Reindex recursively discovers `.md` files against configured search-root globs.
- [ ] AC-2 — Rescan removes vanished documents; attachment rows survive.
- [ ] AC-3 — Document `root` derived purely from the matched search-root glob.
- [ ] AC-4 — `.gitignore`d paths excluded from discovery.
- [ ] AC-5 — Default search-root glob applies when unconfigured.
- [ ] AC-6 — Per-repo search-root config persists and takes effect on next reindex.
- [ ] AC-7 — A glob escaping `clonePath` is rejected with 422.
...
- [ ] AC-41 — No file outside `clonePath` is ever read/indexed.
