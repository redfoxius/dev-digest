...
- Status: draft — all six §13 clarification rows resolved with the requester (2026-08-20); no open `[NEEDS CLARIFICATION]` markers remain. Ready for `implementation-planner`.
- Version: 0.1
...
sequenceDiagram
    ...
    participant Cache as pr_risk_brief (Postgres)
...
- AC-1 (Event-driven): WHEN a client sends `GET /pulls/:id/brief` for a PR with a persisted risk brief, the system shall respond `200` with that persisted `RiskBrief`, without calling the LLM. Verify: seed a `pr_risk_brief` row, GET returns it verbatim; mocked LLM adapter records zero calls.
...
- AC-10 (Unwanted behavior): IF a generated `risks[].file_refs` entry or `review_focus[].file` entry does not match a path present in the PR's diff file list or the blast radius's `changed_symbols`/`downstream` file/endpoint set, THEN the system shall drop that entry before persisting — never surface an invented path. Verify: unit test with a mocked LLM response containing one fabricated file path asserts it is absent from the persisted `risks`/`review_focus` arrays, while entries with real paths survive.
...
- AC-29 (Ubiquitous): The system shall add a new similarity-search
  ...
  the top-`K`. WHILE the repo's `embedResolution.status` is not `'ready'`
  (`context-docs/service.ts:198`), the system shall return an empty
  result without error. Verify: unit test seeds chunks with known
  embeddings and a query embedding, asserts top-`K` ordering matches
  expected cosine-similarity ranking; a second test with `status:
...
### 6.4 Rate limiting
- AC-15 (Ubiquitous): The system shall rate-limit `POST /pulls/:id/brief` to at most 10 requests per minute per workspace, mirroring `POST /pulls/:id/intent/derive`'s existing config. Verify: an 11th POST within 60 seconds from the same workspace returns `429`.
...
- AC-22 (Ubiquitous): `GET /pulls/:id` shall include a new `risk_level: RiskSeverity | null` field, sourced from the PR's persisted risk brief (`null` when none exists yet), computed alongside the route's existing `prBrief` aggregate (`server/src/modules/pulls/routes.ts:282-296`) via one additional repository read. Verify: unit test seeds a `pr_risk_brief` row for a PR, asserts `GET /pulls/:id`'s response includes the matching `risk_level`.
- AC-23 (Event-driven): WHEN a PR's `GET /pulls/:id` response has a non-null `risk_level`, the client shall render a color-coded risk badge (high/medium/low, reusing the same severity color mapping `IntentCard`'s `RiskChips` already uses) in `PrBriefBanner`, alongside its existing verdict/score/findings. Verify: component test with `risk_level: "high"` on the `pr` prop asserts a badge renders using the "high"-severity color token.
...
- N/A (localization): `OverviewTab` and its existing `IntentCard` deliberately use hardcoded English strings today, not `next-intl` (`client/INSIGHTS.md`, 2026-08-09 entry) — the new card follows that same local, already-accepted convention rather than introducing a one-off i18n island; a future Overview-tab-wide i18n pass, if it happens, covers this card too at that point.
...
**New entity: `pr_risk_brief`** (one row per PR — mirrors `pr_intent`'s
single-row-per-PR shape, `server/src/db/schema/reviews.ts:96-138`).

| Field | Type | Notes |
|---|---|---|
| `pr_id` | uuid, PK, FK → pull request | `ON DELETE CASCADE`, mirroring `pr_intent`. |
| `what` | text | What the PR changes, mechanically. Capped at 600 chars (AC-12). |
| `why` | text | Why the change was made. Capped at 600 chars (AC-12). |
| `risk_level` | text enum (`high`/`medium`/`low`) | Model-judged, first-class field (§4). |
| `risks` | jsonb (`Risk[]`) | Reuses the existing `Risk` shape (`kind`/`title`/`explanation`/`severity`/`file_refs`); capped at 8 entries (AC-12). |
| `review_focus` | jsonb (`ReviewFocusItem[]`) | New shape: `{file, line, reason}`; capped at 8 entries (AC-12). |
| `pr_head_sha` | text | The PR's `head_sha` this brief was generated against — the cache-freshness fingerprint (AC-4/AC-5). |
| `provider` / `model` | text | Which provider/model produced it (audit). |
| `tokens_in` / `tokens_out` | integer | From the LLM call's usage, same fields every other LLM integration in this codebase records. |
| `cost_usd` | numeric, nullable | Estimated cost of the one call, null when unknown — same nullability convention as `reviews.cost_usd`. |
| `generated_at` | timestamp | When this row was (last) written. |

**Lifecycle:** created on first successful generation; **overwritten in
place** (not superseded by a new row, unlike `blast_summary`'s
`(pr_id, indexed_sha)` composite-key design) on every subsequent
successful generation, whether triggered by staleness (AC-5) or force
(AC-6); left untouched on any degraded/failed attempt (AC-14); deleted
only via the PR's own cascade delete.
...
- [ ] AC-10 — Fabricated `risks[].file_refs`/`review_focus[].file` entries are dropped before persisting.
...
- [ ] AC-23 — `PrBriefBanner` renders a color-coded risk badge when `risk_level` is non-null.
...
- [ ] AC-29 — `context-docs` gains a top-K cosine-similarity search over indexed chunks; empty result when embeddings aren't ready.
