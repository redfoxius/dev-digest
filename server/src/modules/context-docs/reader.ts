 ...
 * clone root + configured globs, returns discovered docs; the only side
 * effect is reading the repo's own working tree (never writes, never
 * touches the DB — that's `repository.ts`'s job).
 */

/** Default search-root glob (AC-5) — applies whenever `repos.context_search_globs` is null. */
 ...
 * of the three known root names wins. A fully custom glob that never routes
 * through `specs/`, `docs/`, or `insights/` still has to land in one of the
 * three fixed DB values — this defaults such a path to `docs`, the most
 * general of the three (documented judgment call; the default glob, and any
 * custom glob that mirrors its shape, always hits the segment match above).
 */
...
/**
 * Recursively scans `clonePath` for `.md` files matching `globs`, returning
 * one `DiscoveredDoc` per match — sorted by path for a stable, testable
 * result. `EXCLUDED_DIRS` are pruned during the walk itself (never even
 * read), same posture as `repo-intel/pipeline/walk.ts`.
 */
export async function discoverContextDocs(
  ...
  const matched = micromatch(candidates, globs, { dot: false });
